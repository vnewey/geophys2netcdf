...
for field_list in [line.strip().split(',') for line in csv_file]:
    ...
    if (os.path.isfile(nc_path) and uuid
        # and re.search('^/g/data1/rr2/gravity/', nc_path) # Include all datasets in /g/data1/rr2/gravity/
                # and (not re.search('^/g/data1/rr2/gravity/National', nc_path)) #
            # Exclude all datasets in /g/data1/rr2/gravity/National
            # Only work with datasets in their own directory
        and (os.path.splitext(os.path.basename(nc_path))[0] == os.path.basename(os.path.dirname(nc_path)))
        ):
